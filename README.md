...
- **Xはログインが必要なため、さらにリスクがあります。** cookieはセキュリティ上
  リポジトリに保存できないため、30分おきの実行のたびに「本物のXアカウントのID/パスワードで
  毎回ログインし直す」動作になります。データセンターのIPアドレスからの頻繁なログインは
  Xの不正利用検知に引っかかりやすく、**最悪アカウントが一時制限・凍結されるリスクがあります**。
  心配な場合は `X_USERNAME` 等のSecretsを設定しない(Xの自動収集自体を使わない)ことをおすすめします。
- TikTokもGitHub側のIPからはBot対策に引っかかりやすく、失敗しやすいです。
...
| X | `twikit` (要ログイン、非公式) | 低 | 低(アカウント制限リスクあり) |
| TikTok | `TikTokApi` + Playwright (要ms_token、非公式) | 低 | 低 |
